/*
  Shared DOM helpers for the popup and the library view.

  The safety rule of the whole UI lives here: text becomes textContent,
  never markup. The single innerHTML sink is icon(), and it accepts
  only the static constants below — nothing from the network, the page,
  or the player's own notes can reach it.
*/
...
/** Blob download via a temporary anchor — no downloads permission needed. */
...
export async function copyText(text: string): Promise<boolean> {
  try {
    ...
    // No transient activation (or an older context): the selection route.
    try {
